...
def build_tearsheet(df, prices, regimes, weights, backtest,
    ...
    reg_colors = {0: C_BULL, 1: C_EW, 2: C_CRISIS}
    ...
    fig.legend(handles=[mpatches.Patch(color=C_BULL,   alpha=0.7, label="Bull"),
                        mpatches.Patch(color=C_EW,     alpha=0.7, label="Bear"),
                        mpatches.Patch(color=C_CRISIS, alpha=0.7, label="Crisis")],
               loc="upper right", bbox_to_anchor=(0.97, 0.955), fontsize=9,
               framealpha=0.2, labelcolor="white", facecolor="#1e293b", edgecolor="#334155")

    def style(ax, title="", ylabel="", xlabel="Date"):
    ...
    shade(ax1, regime_bt)
    ax1.set_yscale("log")
    style(ax1, title="Equity Curves — Growth of $1 (log scale)", ylabel="Portfolio Value ($)")
    ...
    for m, color in [(m_s, C_STRAT), (m_6040, C_6040), (m_ew, C_EW)]:
        ...
        ax1.annotate(f"${fv:.2f}", xy=(m["cumulative"].index[-1], fv),
                     xytext=(8, 0), textcoords="offset points",
                     color=color, fontsize=8, va="center", fontweight="bold")

    # Drawdown
    ax2 = fig.add_subplot(gs[1, :])
    def dd(cum): return (cum - cum.cummax()) / cum.cummax()
    ax2.fill_between(m_s["cumulative"].index, dd(m_s["cumulative"]).values, 0,
                     color=C_STRAT, alpha=0.35, label="Strategy")
    ax2.plot(dd(m_6040["cumulative"]), color=C_6040, lw=1.2, linestyle="--",
             label="60/40", alpha=0.85)
    ax2.plot(dd(m_ew["cumulative"]),   color=C_EW,   lw=1.2, linestyle=":",
             label="Equal Weight", alpha=0.85)
    shade(ax2, regime_bt)
    ax2.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1))
    style(ax2, title="Drawdown", ylabel="Drawdown")
    ax2.legend(fontsize=9, framealpha=0.3, facecolor="#1e293b",
               labelcolor="white", edgecolor="#334155", loc="lower left")

    # Rolling Sharpe
    ax3 = fig.add_subplot(gs[2, :2])
    def rs(r): return r.rolling(252).mean() / r.rolling(252).std() * np.sqrt(252)
    ax3.plot(rs(strat_rets), color=C_STRAT, lw=1.5, label="Strategy")
    ax3.plot(rs(ret_6040.reindex(strat_rets.index).fillna(0)),
    ...
    shade(ax3, regime_bt)
    style(ax3, title="Rolling 1-Year Sharpe Ratio", ylabel="Sharpe")
    ...
    _, texts, _   = ax6.pie(regime_counts.values,
